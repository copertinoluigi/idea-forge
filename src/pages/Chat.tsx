  ...
  return (
    <div className="h-full w-full flex bg-gray-950 text-white overflow-hidden font-sans relative">
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-gray-900 border-r border-gray-800 transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-gray-800 space-y-1 bg-gray-950/50">
            ...
            <Button onClick={onNavigateToSettings} variant="ghost" className="w-full justify-start text-sm text-gray-400 font-bold"><Settings className="h-4 w-4 mr-2" /> Settings</Button>
            <Button onClick={() => signOut()} variant="ghost" className="w-full justify-start text-sm text-red-400"><LogOut className="h-4 w-4 mr-2" /> Logout</Button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gray-950 h-full relative">
        ...
        <div className="p-4 md:p-6 bg-gray-950 border-t border-gray-800 relative">
          <form onSubmit={handleSend} className="max-w-4xl mx-auto flex gap-2 items-center relative" autoComplete="off">
            {showEmojiPicker && (
