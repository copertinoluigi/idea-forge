...
export function Chat({ activeRoomId, onRoomChange, onNavigateToSettings, onNavigateToAdmin, onSummarize, onDevelop }: ChatProps) {
  ...
  // FIX: Definizione activeRoom per l'header e la logica
    ...
    try {
      ...
      if (!privateConsole) {
        const { data: newRoom, error: rErr } = await supabase.from('rooms').insert({
          name: 'La mia Console', is_private: true, created_by: user.id, ai_provider: profile?.ai_provider || 'google-flash'
        }).select().single();
  ...
  const loadMessages = async (roomId: string) => {
    const { data } = await supabase.from('messages').select('*, profiles(display_name)').eq('room_id', roomId).order('created_at', { ascending: true });
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { error } = await supabase.from('messages').insert({ user_id: user.id, content, room_id: currentRoomId });
      if (error) throw error;
      if (activeRoom?.is_private) {
        const history = messages.map(m => ({ role: m.is_system ? 'assistant' : 'user', content: m.content }));
        history.push({ role: 'user', content });
        const reply = await chatWithAI({ messages: history, provider: activeRoom.ai_provider, apiKey: activeRoom.encrypted_api_key || profile?.encrypted_api_key || '' });
      ...
      <aside className="w-64 border-r border-gray-800 bg-gray-900/50 flex flex-col hidden md:flex">
        <div className="p-6">
          <div className="flex items-center gap-2 mb-8 justify-center">
            ...
            <h1 className="font-black text-xl italic uppercase text-white tracking-tighter tracking-widest">IdeaForge</h1>
          </div>
          ...
          <nav className="space-y-1 overflow-y-auto">
            {roomsLoading ? <div className="flex justify-center p-4"><Loader2 className="animate-spin h-4 w-4 text-gray-700" /></div> : rooms.map(room => (
        ...
        <header className="h-16 border-b border-gray-800 bg-gray-900/50 backdrop-blur-xl px-6 flex items-center justify-between">
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <h2 className="font-bold text-sm text-white uppercase tracking-tighter italic">{activeRoom?.name || 'Seleziona stanza'}</h2>
              {!activeRoom?.is_private && activeRoom?.join_code && (
                <button onClick={copyJoinCode} className="bg-gray-800 text-[10px] px-2 py-0.5 rounded border border-gray-700 text-gray-400 hover:text-white flex items-center gap-1 transition-colors">#{activeRoom.join_code} <Copy className="h-2.5 w-2.5" /></button>
              )}
            </div>
            <span className="text-[9px] text-violet-400 font-black uppercase tracking-[0.2em]">{activeRoom?.ai_provider} active</span>
          </div>
          <div className="flex items-center gap-3">
            <Button onClick={() => { if(!isSelectionMode) setIsSelectionMode(true); else { onSummarize(messages.filter(m => selectedMessageIds.includes(m.id))); setIsSelectionMode(false); setSelectedMessageIds([]); } }} size="sm" className={`${isSelectionMode ? 'bg-green-600' : 'bg-violet-600'} text-white rounded-full font-black px-5 text-[10px] uppercase tracking-widest`}><Sparkles className="mr-2 h-3.5 w-3.5" /> {isSelectionMode ? `Confirm (${selectedMessageIds.length})` : 'Summarize'}</Button>
