...
export function Chat({ activeRoomId, onRoomChange, onNavigateToSettings, onNavigateToAdmin, onSummarize, onDevelop }: ChatProps) {
  ...
  const loadRooms = async () => {
    if (!user) return;
    const { data } = await supabase.from('rooms').select('*, room_members!inner(user_email)').eq('room_members.user_email', user.email);
  ...
  const isAdmin = user?.email === 'unixgigi@gmail.com';
      ...
      <aside className="w-64 border-r border-gray-800 bg-gray-900/50 flex flex-col hidden md:flex">
        <div className="p-6">
          <div className="flex items-center gap-2 mb-8">
            ...
            <h1 className="font-black text-xl italic tracking-tighter uppercase">IdeaForge</h1>
          </div>
          <div className="flex items-center justify-between mb-4 px-2">
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Workspace</p>
            <Button onClick={() => setIsAddRoomOpen(true)} variant="ghost" size="icon" className="h-5 w-5 hover:text-violet-400"><Plus className="h-4 w-4" /></Button>
          </div>
          <nav className="space-y-1">
            {rooms.map(room => (
              <button key={room.id} onClick={() => onRoomChange(room.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${activeRoomId === room.id ? 'bg-violet-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-800'}`}>
        ...
        <div className="mt-auto p-4 border-t border-gray-800 space-y-1">
          {isAdmin && <Button onClick={onNavigateToAdmin} variant="ghost" className="w-full justify-start text-xs text-emerald-400 hover:text-emerald-300"><ShieldCheck className="h-4 w-4 mr-2" /> Gestione Riservata</Button>}
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gray-950">
        <header className="h-16 border-b border-gray-800 bg-gray-900/50 backdrop-blur-xl px-6 flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <span className="text-[9px] text-gray-500 uppercase tracking-widest">{activeRoom?.ai_provider || 'system'} engine</span>
          </div>
          <div className="flex items-center gap-3">
            <Button onClick={() => { if(!isSelectionMode) setIsSelectionMode(true); else { onSummarize(messages.filter(m => selectedMessageIds.includes(m.id))); setIsSelectionMode(false); setSelectedMessageIds([]); } }} size="sm" className={`${isSelectionMode ? 'bg-green-600' : 'bg-violet-600'} rounded-full font-bold px-4 transition-all`}><Sparkles className="mr-2 h-4 w-4" /> {isSelectionMode ? `Conferma (${selectedMessageIds.length})` : 'Summarize'}</Button>
        ...
        <form onSubmit={handleSend} className="p-6 bg-gray-900/30 border-t border-gray-800">
          <div className="max-w-4xl mx-auto flex gap-4">
            <Textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder={isSelectionMode ? "Seleziona i messaggi sopra e premi Conferma" : "Condividi un'idea..."} disabled={isSelectionMode || !activeRoomId} className="flex-1 bg-gray-800/50 border-gray-700 text-white rounded-2xl focus:ring-violet-500/50 min-h-[52px] resize-none" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(e); } }} />
            <Button type="submit" disabled={loading || !newMessage.trim() || isSelectionMode || !activeRoomId} className="bg-violet-600 rounded-2xl h-[52px] px-6 shadow-lg">
